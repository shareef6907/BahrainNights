...
export default function CinemaTrailersAdminPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch featured cinema trailers
      const featuredRes = await fetch('/api/admin/cinema/trailers');
      const featuredData = await featuredRes.json();
      setFeaturedTrailers(featuredData.trailers || []);
      setTableExists(featuredData.tableExists !== false);

      // Fetch all movies with trailers (only scraped movies)
  ...
  const createTable = async () => {
    ...
    try {
      const res = await fetch('/api/admin/cinema/trailers/init', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        setTableExists(true);
        fetchData();
      } else {
        alert('Failed to create table. Please run the SQL manually in Supabase:\n\n' + (data.sql || data.error));
      }
    } catch (error) {
  ...
  const addTrailer = async () => {
    ...
    try {
      await fetch('/api/admin/cinema/trailers', {
        method: 'POST',
  ...
  const removeTrailer = async (id: string) => {
    if (!confirm('Remove this trailer from cinema hero?')) return;

    try {
      await fetch(`/api/admin/cinema/trailers/${id}`, { method: 'DELETE' });
      fetchData();
    } catch (error) {
  ...
  const toggleActive = async (id: string, isActive: boolean) => {
    try {
      await fetch(`/api/admin/cinema/trailers/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const moveTrailer = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      ...
      await fetch('/api/admin/cinema/trailers/reorder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl font-bold">Cinema Hero Trailers</h1>
            </div>
            <p className="text-gray-400">Manage trailers shown on the cinema page hero section</p>
          </div>
          <Link
            href="/admin/cinema"
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-300 mb-4">
                  The cinema_featured_trailers table doesn&apos;t exist yet. Click below to create it.
                </p>
                <button
                  onClick={createTable}
          ...
          <>
            ...
            <div className="bg-gray-900 rounded-xl p-6 mb-8 border border-gray-800">
              <h2 className="font-bold mb-4 flex items-center gap-2">
                ...
                Add Trailer to Cinema Hero
              </h2>
              <div className="flex gap-4">
                <select
                  value={selectedMovie}
                  onChange={(e) => setSelectedMovie(e.target.value)}
                  className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-red-500 focus:outline-none"
                >
                  <option value="">Select a movie with trailer...</option>
                  {availableToAdd.map((movie) => (
                    <option key={movie.id} value={movie.id}>
                      {movie.title} {movie.tmdb_rating ? `(${movie.tmdb_rating.toFixed(1)}⭐)` : ''}
                    </option>
                  ))}
                </select>
                <button
                  ...
                  className="bg-red-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-colors"
                >
                  <Plus size={18} />
                  Add
                </button>
              ...
              {availableMovies.length === 0 && (
                <p className="text-gray-500 text-sm mt-3">
                  No movies with trailers found. Make sure movies have trailer URLs set.
                </p>
              )}
            </div>
            ...
            <div className="bg-gray-900 rounded-xl overflow-hidden border border-gray-800">
              <div className="p-4 border-b border-gray-800 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">First trailer plays first on the cinema page</p>
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="p-8 text-center text-gray-500">
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    Without featured trailers, the cinema page will show top-rated movies with trailers.
                  </p>
                </div>
              ) : (
                  ...
                  {featuredTrailers.map((trailer, index) => (
                    ...
                    >
                      ...
                      {trailer.movie?.poster_url ? (
                        <img
                          src={trailer.movie.poster_url.startsWith('http')
                            ? trailer.movie.poster_url
                            : `https://image.tmdb.org/t/p/w500${trailer.movie.poster_url}`}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {trailer.movie?.tmdb_rating && (
                          <p className="text-xs text-yellow-500 mt-1">
                            ⭐ {trailer.movie.tmdb_rating.toFixed(1)}
                          </p>
                        )}
                      </div>
            ...
            <div className="mt-6 p-4 bg-gray-900/50 rounded-xl border border-gray-800">
              ...
              <ul className="text-sm text-gray-500 space-y-1">
                <li>
                  <span className="text-red-400">1.</span> Select movies from the dropdown to add them as featured trailers
                </li>
                <li>
                  <span className="text-red-400">2.</span> Use the arrows to reorder - first trailer plays first
                </li>
                <li>
                  <span className="text-red-400">3.</span> Use the eye icon to temporarily hide a trailer without removing it
                </li>
                <li>
                  <span className="text-red-400">4.</span> Trailers auto-advance every 25 seconds on the cinema page
                </li>
                <li>
                  <span className="text-red-400">5.</span> If no trailers are set, top-rated movies with trailers will be shown
                </li>
              </ul>
            </div>
          </>
