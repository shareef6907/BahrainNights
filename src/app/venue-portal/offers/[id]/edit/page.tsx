...
const statusOptions = [
  ...
  { value: 'paused', label: 'Paused', color: 'bg-yellow-500/20 text-yellow-400' },
  { value: 'expired', label: 'Expired', color: 'bg-red-500/20 text-red-400' },
...
export default function EditOfferPage() {
  ...
  useEffect(() => {
    async function loadOffer() {
      try {
        const response = await fetch(`/api/venue-portal/offers/${offerId}`);
        if (!response.ok) {
        ...
        setFormData({
          ...
          is_ongoing: offer.is_ongoing ?? true,
          whats_included: offer.whats_included?.length > 0 ? offer.whats_included : [''],
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      console.log(`[Offer Edit] Compressing ${file.name}: ${(file.size / 1024).toFixed(0)}KB`);
      let compressedFile: File;
      try {
        compressedFile = await compressImage(file);
        console.log(`[Offer Edit] Compressed to: ${(compressedFile.size / 1024).toFixed(0)}KB`);
      } catch (compressError) {
        console.error('[Offer Edit] Compression failed:', compressError);
        compressedFile = file;
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Offer Updated!</h2>
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-white">Edit Offer</h1>
            <p className="text-gray-400 mt-1">Update your offer details.</p>
          </div>
        </div>
        <button
          type="button"
