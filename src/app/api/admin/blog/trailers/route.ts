...
export async function GET() {
  try {
    ...
    if (error) {
      ...
      console.error('Featured trailers fetch error:', error);
    ...
    const trailersWithMovies = await Promise.all(
      (trailers || []).map(async (trailer) => {
        ...
        const { data: movie } = await (supabase as any)
          .from('movies')
          .select('id, title, poster_url, backdrop_url, trailer_url, trailer_key, synopsis, genre')
    ...
    console.error('Featured trailers exception:', err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Insert featured trailer error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ trailer: data });
  } catch (err) {
    console.error('POST featured trailer exception:', err);
