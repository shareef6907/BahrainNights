...
// Fixes timezone issue in event times
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      success: true,
      stats,
      message: `TIMEZONE FIX COMPLETE! Synced ${stats.fetched} events: ${stats.inserted} inserted, ${stats.updated} updated, ${stats.errors} errors`,
      note: 'DELETE THIS ENDPOINT AFTER USE: src/app/api/admin/sync-platinumlist-fix/route.ts'
    });
  } catch (error) {
