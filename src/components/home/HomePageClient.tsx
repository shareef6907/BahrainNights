...
interface HomePageClientProps {
  ...
  initialLiveEventCounts?: { local: number; international: number };
}

export default function HomePageClient({ initialMovies, initialStats, initialTodayEvents, initialInternationalEvents, initialHappeningNowEvents, initialSurpriseData, initialTrendingData, initialLiveEventCounts }: HomePageClientProps) {
  ...
  return (
    <div className="bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white min-h-screen">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-[70] transition-all duration-300 ${isScrolled ? 'bg-slate-950/95 backdrop-blur-xl border-b border-white/10 shadow-xl shadow-black/20' : ''}`}>
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center space-x-2">
              ...
              <Link
                href="/register-venue"
                className="flex items-center space-x-1.5 bg-yellow-500 text-black px-3 py-2 rounded-full text-sm font-semibold hover:shadow-lg hover:shadow-orange-500/25 hover:scale-105 transition-all duration-200"
              >
          ...
          >
              <div className="px-4 py-4 space-y-1">
                {menuItems.map((item) => (
                  <div key={item.name} className="border-b border-white/5 last:border-0">
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${mobileAccordion === item.name ? 'rotate-180 text-yellow-500' : ''}`}
                      />
                    </button>
                ...
                <div className="pt-4 space-y-3">
                  <Link
                    href="/register-venue"
                    className="w-full flex items-center justify-center space-x-2 bg-yellow-500 text-black px-6 py-4 rounded-xl font-semibold"
                    onClick={() => setMobileMenuOpen(false)}
                  >
      ...
      <section className="relative pt-32 pb-32 px-4 min-h-[600px] md:min-h-[700px]">
        ...
        <div className="absolute inset-0 w-full h-full">
          ...
          <video
            ...
            autoPlay
            preload="metadata"
            poster="/header-poster-v2.webp"
            disablePictureInPicture
            className="absolute inset-0 w-full h-full object-cover"
            style={{ objectPosition: 'center center' }}
          >
            <source src="/header-1080p-hq-v2.webm" type="video/webm" />
            <source src="/header-1080p-hq-v2.mp4" type="video/mp4" />
          </video>
                ...
                >
                  ...
                  {movie.trailer_url && (
                    <button
                      ...
                      className="absolute top-2 right-2 md:hidden p-4 bg-black/80 backdrop-blur-sm rounded-full text-yellow-500 active:bg-pink-500 active:text-black transition-all duration-150 z-10 touch-manipulation"
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      ...
                      {movie.trailer_url && (
                        <button
                          ...
                          className="mt-3 hidden md:flex items-center gap-2 px-4 py-2 bg-yellow-500 text-black rounded-full font-medium hover:bg-yellow-400 transition-colors"
                        >
