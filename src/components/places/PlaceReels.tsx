...
export interface VenueReel {
  ...
  video_url?: string | null; // S3 URL for autoplay video
  ...
  return (
    <>
      ...
      >
        ...
        {/* Reels Grid - Video autoplay or static thumbnails */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {activeReels.map((reel, index) => (
            ...
            >
              {/* If video_url exists, use autoplay ReelPlayer */}
              {reel.video_url ? (
                <ReelPlayer
                ...
                /* Fallback: Static thumbnail that opens Instagram */
                <button
                  onClick={() => handleReelClick(index)}
                  className="group relative w-full h-full rounded-xl overflow-hidden border border-white/10 hover:border-pink-500/50 transition-all"
                >
